...
import { withTimeout, capture } from "../utils.js";
  ...
  return await withTimeout(
    ...
    "Read file handler operation",
    {
      content: [
        {
          type: "text",
          text: `Operation timed out after ${
            HANDLER_TIMEOUT / 1000
          } seconds. The file might be too large or on a slow/unresponsive storage device.`,
        },
      ],
    }
  );
  ...
  const textSummary = fileResults
    .map((result) => {
      if (result.error) {
        ...
        return `${result.path}: ${result.mimeType} ${
          result.isImage ? "(image)" : "(text)"
        }`;
      } else {
...
export async function handleCreateDirectory(
  ...
  try {
    ...
    return {
      content: [
        { type: "text", text: `Successfully created directory ${parsed.path}` },
      ],
    };
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
  ...
  try {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Successfully moved ${parsed.source} to ${parsed.destination}`,
        },
      ],
    };
  } catch (error) {
